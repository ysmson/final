...
public class Video {
    ...
    private String id;               // ✅ 資料庫主鍵 ID（varchar 型別，可能是 UUID 或流水號）
    private String videoId;          // ✅ YouTube 影片 ID（例如："dQw4w9WgXcQ"）
    private String title;            // ✅ 影片標題（例如："介紹台中美食"）
    private String description;      // ✅ 影片描述文字（說明欄內容）
    private String thumbnailUrl;     // ✅ YouTube 自動產生的縮圖 URL（可顯示預覽圖）
    private String publishedAt;      // ✅ 影片發佈時間（ISO 格式，例如："2024-06-01T08:00:00Z"）

    // ✅ 無參數建構子（必要於 JSON 解析、資料綁定等框架）
    public Video() {}

    /**
     * ✅ 有參數建構子：快速建立影片資料物件（用於測試或初始化）
     * @param videoId YouTube ID
     * @param title 標題
     * @param description 描述
     * @param thumbnailUrl 縮圖連結
     * @param publishedAt 發布時間（ISO 字串格式）
     */
    ...
    // ✅ 以下為標準的 Getter / Setter，用於物件屬性操作與封裝（JSP、Controller、JSON）
     ...
     * @return YouTube 影片播放網址（可直接用於 <iframe src="...">）
     */
